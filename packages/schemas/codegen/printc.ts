...
function printCInterfaceCombinator(c: gen.InterfaceCombinator, i: number, recursion?: gen.Recursion): string {
  let s = 't.TypeC<{\n';
...
function printCPartialCombinator(c: gen.PartialCombinator, i: number, recursion?: gen.Recursion): string {
  let s = 't.PartialC<{\n';
...
function printCTypesCombinator(types: gen.TypeReference[], i: number, recursion?: gen.Recursion): string {
  const indentation = indent(i + 1);
  return types.map((t) => `${indentation}${printC(t, i, recursion)}`).join(`,\n`);
}

function printCUnionCombinator(c: gen.UnionCombinator, i: number, recursion?: gen.Recursion): string {
...
function printCKeyofCombinator(c: gen.KeyofCombinator, i: number): string {
  return printC(gen.unionCombinator(c.values.map((value) => gen.literalCombinator(value))), i);
}

function printCArrayCombinator(c: gen.ArrayCombinator, i: number, recursion?: gen.Recursion): string {
  return `t.ArrayC<${printC(c.type, i, recursion)}>`;
}

function printCExactCombinator(c: gen.ExactCombinator, i: number, recursion?: gen.Recursion): string {
  return printC(c.type, i, recursion);
}

function printCStrictCombinator(c: gen.StrictCombinator, i: number, recursion?: gen.Recursion): string {
  let s = 't.TypeC<{\n';
  s += c.properties.map((p) => printCProperty(p, i + 1, recursion)).join(',\n');
  s += `\n${indent(i)}}>`;
  return s;
...
function printCReadonlyCombinator(c: gen.ReadonlyCombinator, i: number, recursion?: gen.Recursion): string {
  return `t.ReadonlyC<${printC(c.type, i, recursion)}>`;
}

function printCBrandCombinator(bc: gen.BrandCombinator, i: number, recursion?: gen.Recursion): string {
...
function printCTupleCombinator(c: gen.TupleCombinator, i: number, recursion?: gen.Recursion): string {
  const indentation = indent(i + 1);
  let s = 't.TupleC<[\n';
...
export function printC(node: gen.Node, i: number = 0, recursion?: gen.Recursion): string {
  switch (node.kind) {
    case 'Identifier':
      return node.name + 'C';
    case 'StringType':
...
/* The following formatting rules were copied from io-ts-codegen */
