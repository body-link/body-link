...
import { Signal, SignalModifier, SignalWave } from './types';

export const signalWave$: Subject<SignalWave> = new Subject<SignalWave>();
 ...
 * An optional function to map payload on other value and produce custom value for return from `cs`
 * after the call.
...
export function cs<P1, P2, R>(modifier?: SignalModifier<P1, P2, R>): Signal<P1, P2, R> {
  const signal = (
    typeof modifier === 'function'
      ...
      : (payload) => signalWave$.next({ signal, payload })
  ) as Signal<unknown>;
