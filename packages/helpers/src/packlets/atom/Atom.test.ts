...
function testDerivedAtom(
  ...
  describe('two atoms subscriptions', () => {
    ...
    const v1 = createDerived(
      a,
      (x) => x + 1,
      (x) => viewFnCalls1.push(x)
    );
    const v2 = v1.view((x) => {
      viewFnCalls2.push(x);
      return x + 5;
...
describe('atom', () => {
  ...
  describe('lens', () => {
    describe('lensed atoms cached', () => {
      ...
      const a1$ = atom.lens('a');
      const b1$ = atom.lens('a');
      expect(a1$).toStrictEqual(b1$);
      const a2$ = atom.lens('a', 'b');
      const b2$ = atom.lens('a', 'b');
      expect(a2$).toStrictEqual(b2$);
      const lens = Lens.create<{ a: { b: { c: number } } }, number>(
        (x) => x.a.b.c,
        (v, s) => s
      );
      const a3$ = atom.lens(lens);
      const b3$ = atom.lens(lens);
      expect(a3$).toStrictEqual(b3$);
    });

    describe('lensed, nested safe key', () => {
      testAtom((x) => {
        const source = Atom.create({ a: { b: { c: x } } });
        return source.lens('a', 'b', 'c');
      });
    });
    ...
    describe('lensed, safe key, chained lenses', () => {
      testAtom((x) => {
        const source = Atom.create({ a: { b: { c: x } } });
        return source.lens('a').lens('b').lens('c');
      });
    });
  ...
  describe('combine', () => {
    describe('constant', () => {
      const combined = Atom.combine(Atom.create(1), Atom.create(false), Atom.create('test'), (x, y, z) =>
        y && x < 0 ? z.toUpperCase() : 'NO'
      );
